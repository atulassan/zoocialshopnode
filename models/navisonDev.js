...
Navison = {
    async insert(req) {
        ...
        if(navisonData.hasOwnProperty('status') && navisonData.status == 200 ) {
            ...
            for (const [indx, apiData] of navisonData.data.entries()) { 
                ...
                let newProduct = await postDevWCApiAsync("products", productData);
                ...
                if(newProduct.hasOwnProperty('code') && newProduct.code == "product_invalid_sku") {
                    ...
                    var updProduct = await putDevWCApiAsync("products/"+updId, productData);
    ...
    async productData(apiData) {
        ...
        if(apiData.brand != null && apiData.brand != "") {
            ...
            let newBrand = await postDevWCApiAsync("products/tags", branData);
        ...
        if(discountData.hasOwnProperty('status') && discountData.status == 200 && discountData.data[0].length > 0 && discountData.data[0][0].hasOwnProperty('line discount _')) {
            ...
            wcData.meta_data.push({ "key": "discountperiod", "value": discountData.data[0][0] });
        } else {
    ...
    async getNavisionItem(req) {
        ...
        /*let wcProduct = await getDevWCApiAsync("products/", {"sku": psku});
        if(wcProduct.length > 0 && wcProduct[0].hasOwnProperty('id')) {
            let updateData = await putDevWCApiAsync("products/"+wcProduct[0].id, productData);
    ...
    async getCategories(mainCategory, childCategory, itemcode, variantcode, categories, cats) {
        ...
        if(childCategory !== null && childCategory !== "" && (mainCategory === "hund" || mainCategory === "katze" || mainCategory === "andere")) {
            ...
            if(category.length > 0) {
                ...
                let createCategory = await postDevWCApiAsync("products/categories", cData);
    ...
    async getNoImagesProduct(req) {
        ...
        while (page !== false) {
            ...
            let products = await getDevWCApiAsync(`products?images=no&per_page=500&page=${page}`);
        ...
        //allProducts = await getDevWCApiAsync(`products?per_page=200&page=${page}`);
    ...
    async navisionTest() {
        ...
        /*var options = {
                        url: "https://navshop.provet.ch/api/v1/item/all", 
                        key: 'szYITNzF1WPzBf61SKW9pYwT-3K2RY',
                        formData: {SortByLastModified: 123}
                    };
        //var options = { url: "https://navshop.provet.ch/api/v1/item/getItems", key: 'szYITNzF1WPzBf61SKW9pYwT-3K2RY' };
        //var options = { url: "https://navision.oneix.dev/api/v1/item/all", key: 'YMv7wakenwoJlN2wrUDa4QM8-Adj28' };
        //var options = { url: "https://navision.oneix.dev/api/v1/item/getItems", key: 'YMv7wakenwoJlN2wrUDa4QM8-Adj28' };
            ...
            for (const [indx, apiData] of navisonData.data.entries()) {
                ...
                if(discountData.hasOwnProperty('status') && discountData.status == 200 && discountData.data[0].length > 0) {
                    discountData.data[0].forEach((discount, el)=> {
                        ...
                        // get highest discount version No
                        discountVersionNo = (discountVersionNo < parseInt(discount["VersionNo"])) ? discount["VersionNo"] : discountVersionNo;
                    });
                ...
                wcData.versionNo = discountVersionNo;

                console.log(wcData);

            }

            console.log("+++++++++++++++++++++ Discount Version NO +++++++++++++++++++++++");
            console.log(discountVersionNo);
            console.log("+++++++++++++++++++++ Discount Version NO +++++++++++++++++++++++");
                ...
                //if(parseInt(apiData.VersionNo) > parseInt(versionNo)) {
                    ...
                    if(discountData.hasOwnProperty('status') && discountData.status == 200 && discountData.data[0].length > 0) {
                        discountData.data[0].forEach((discount, el)=> {
                            ...
                            discountDesc = (discountPercentage < parseInt(discount["line discount _"])) ? discount['Description'] : discount['Description'];
                            //Highest Version No
                    ...
                    let wcProduct = await getDevWCApiAsync("products", rdata);
                    if(wcProduct.length > 0 && wcProduct[0].hasOwnProperty('id')) {
                        let updateData = await putDevWCApiAsync("products/"+wcProduct[0].id, wcData);
                        console.log("+++++++++ Data Updated ++++++++");
    ...
    async filtersUpdate(req) {
        ...
        if(navisonData.hasOwnProperty('status') && navisonData.status == 200 ) {
            ...
            let wcTags = await getDevWCApiAsync("products/tags", treqdata);
            //remove empty tags
            if(wcTags.length > 0 ) {
                for (const [tidx, tag] of wcTags.entries()) {
                    if(!tag.count) {
                        let deleteEmpty = await getDevWCApiAsync("products/tags/"+tag.id, {force: true});
    ...
    async imagesUpdateOld() {

        let data = {};
        var versionNo = ""; 

        const url = "https://zshopdev.ch/wp-json/jwt-auth/v1/token";

        let formdata = {username: 'zboss', password: 'e4Vk?PM7xKKP4HBt1Dm+'};
        ...
        if (fs.existsSync("navimages.json")) {
            let navijson = fs.readFileSync('navimages.json');
            let naviData = JSON.parse(navijson);
            versionNo = naviData.hasOwnProperty('versionNo') ? naviData.versionNo : "";
            console.log(versionNo);      
        }
        ...
        if(navisonData.hasOwnProperty('status') && navisonData.status == 200 ) {
                    ...
                    if(wcProduct.length > 0 && wcProduct[0].hasOwnProperty('id')) {
                        ...
                        if(wcData.hasOwnProperty('images') && wcProduct[0].images.length > 0) {
                            ...
                            for (const [indx, image] of wcProduct[0].images.entries()) {
                                ...
                                if(result.status && result.response.hasOwnProperty('deleted') && result.response.deleted ) {
            ...
            if (fs.existsSync("navimages.json")) {
                //file exists
                fs.writeFile('navimages.json', versionData, (err) => {
                    if (err) throw err;
                    console.log('Data written to file');
                });
            } else {
                fs.writeFileSync('navimages.json', versionData);
            }
